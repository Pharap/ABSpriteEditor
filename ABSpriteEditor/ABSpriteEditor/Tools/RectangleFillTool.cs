...
{
    public class RectangleFillTool : ITool<BitmapEditorPanel>
    {
        ...
        private Rectangle CalculateOverlayAreaRectangle()
        {
            ...
            var endX = PointHelper.Clamp(maxPoint.X + 1, 0, width);
            var endY = PointHelper.Clamp(maxPoint.Y + 1, 0, height);
        ...
        private Rectangle CalculateFillAreaRectangle()
        {
            // Localise the start and end points
            var localStart = this.control.ToLocal(this.startPoint.Value);
            var localEnd = this.control.ToLocal(this.endPoint.Value);
        ...
        private void control_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button.HasFlag(MouseButtons.Left))
            {
                ...
                {
                    // Create a rectangle identifying the area to fill
                    var fillArea = this.CalculateFillAreaRectangle();

                    // Fill the rectangle
                    BitmapHelper.FillRectangle(this.control.Image, fillArea, this.control.ForeColor);
